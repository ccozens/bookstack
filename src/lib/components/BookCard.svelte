...
<article class="flex gap-3 py-3 border-b border-[var(--color-border)] last:border-0 group">
  <!-- Cover thumbnail -->
  <div class="shrink-0 w-10 h-14 rounded-lg overflow-hidden bg-[--color-surface-2] flex items-center justify-center">
    ...
    {:else}
      <svg class="w-5 h-5 text-[var(--color-muted)] opacity-50" fill="currentColor" viewBox="0 0 24 24">
  ...
  <div class="flex-1 min-w-0">
    ...
    {#if book.isbn}
      <p class="text-xs text-[var(--color-muted)] mt-0.5">ISBN: {book.isbn}</p>
    {/if}
    <p class="text-xs text-[var(--color-muted)] mt-1">{formatDate(book.dateRead)}</p>
  </div>
  ...
  {#if $isAuthorised}
    <div class="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
      ...
      <button
        ...
        class="text-xs text-[var(--color-muted)] hover:text-red-400 transition-colors px-2 py-1 rounded hover:bg-[--color-accent-hover]"
